define(["zepto"], function($) {
  var alet_data = {
    ...
    nomore_word_short:
      '<div class="toast_data_short" id="toast_data_short"></div>', //加载到最后的提示
  ...
  //提示文字显示动画
  function nomoredata_show_short(val) {
    $("#toast_data_short").text(val);
      alet_data.left_px = count_leftpx();
    $("#toast_data_short")
      .css({ "z-index": 12, left: alet_data.left_px + "px" })
      .animate({ opacity: 0.85 }, 500, "ease-out");
  }

  //文字显示3s后隐藏
  function alert_word_show_short(val) {
    nomoredata_show_short(val);
